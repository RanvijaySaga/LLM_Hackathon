...
class MerchantDataset(Dataset):
    def __init__(self, merchants, expense_types, tokenizer, max_len):
...
def main():
    # Load dataset (assumed to have 'Merchant' and 'ExpenseType' columns)
    ...
    # Train-test split
    train_merchants, val_merchants, train_labels, val_labels = train_test_split(
    ...
    # Hyperparameters
    MAX_LEN = 64  # Adjust depending on the length of Merchant names
    BATCH_SIZE = 16
    EPOCHS = 3
    LR = 2e-5  # Learning rate

    # Create DataLoaders
    train_dataset = MerchantDataset(train_merchants.tolist(), train_labels.tolist(), tokenizer, MAX_LEN)
    val_dataset = MerchantDataset(val_merchants.tolist(), val_labels.tolist(), tokenizer, MAX_LEN)
    ...
    model.save_pretrained("bert_merchant_classification_model")
    ...
    for i in range(5):  # Show 5 example predictions
        ...
        inputs = tokenizer.encode_plus(merchant, return_tensors="pt", truncation=True, max_length=MAX_LEN,
        ...
        print(
            f"Merchant: {merchant}, True Label: {label_encoder.inverse_transform([label])[0]}, Predicted: {label_encoder.inverse_transform([predicted_label])[0]}")
